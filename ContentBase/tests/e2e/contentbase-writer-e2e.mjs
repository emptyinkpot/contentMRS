 ...
 * 执行: pnpm test:e2e
 * 诊断块/禁忌词表/产物命名+分割规则统一来自 ./diagnostics.mjs。
 */
import fs from "node:fs"
import path from "node:path"
import { fileURLToPath } from "node:url"
import {
  BANNED_NAMES, BANNED_PHRASES, BANNED_TRANSITIONS,
  articleFileName, buildDiagnosticsBlock,
  body, bodyLen, packed, items, conts, DOWNLOADS,
...
function saveArticle(id, topic, content, r, testResult) {
  ...
  fs.writeFileSync(path.join(DOWNLOADS, articleFileName(id, topic)), content + diag, "utf8")
}
