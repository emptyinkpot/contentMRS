...
let buildArticleContextEngine, callSingleWriter, applyPostProcessing;
const compiledPath = path.join(__dirname, 'product', 'novel', 'dist', 'product', 'novel', 'app', 'article', 'context-engine.js');
const modelRuntimePath = path.join(__dirname, 'product', 'novel', 'dist', 'product', 'novel', 'app', 'article', 'model-runtime.js');
const postProcessPath = path.join(__dirname, 'product', 'novel', 'dist', 'product', 'novel', 'app', 'article', 'post-process.js');
if (fs.existsSync(compiledPath)) {
  ({ buildArticleContextEngine } = require(compiledPath));
  ({ callSingleWriter } = require(modelRuntimePath));
  ({ applyPostProcessing } = require(postProcessPath));
} else {
  ...
  ({ callSingleWriter } = require('./product/novel/app/article/model-runtime.ts'));
  ({ applyPostProcessing } = require('./product/novel/app/article/post-process.ts'));
}
...
async function generateArticle(request) {
  ...
  if (body.length > 1000) {
    body = applyPostProcessing(body).result;
  }
